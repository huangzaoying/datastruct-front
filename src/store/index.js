...
const mutations = {
  ...
  GET_work(state, value) {
    ...
    state.workData.workList.splice(state.workData.total, 1000000 - state.workData.total + 1);
  },
  GET_exam(state, value) {

    for (let i = 0; i < value.val.length; i++) {
      state.examData.examList[i] = value.val[i]
    }
    state.examData.total = value.val.length
    state.examData.examList.splice(state.examData.total, 1000000 - state.examData.total + 1);
  },
  GET_log(state, value) {
    for (let i = 0; i < value.length; i++) {
      state.logData.logList[i] = value[i]
    }
    state.logData.total = value.length
    if (state.logData.total >= 10)
      state.logData.logList.splice(0, state.logData.total - 10);
    state.logData.logList.splice(state.logData.total, 1000000 - state.logData.total + 1);
